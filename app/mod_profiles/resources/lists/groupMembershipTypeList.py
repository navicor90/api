# -*- coding: utf-8 -*-

from flask_restful import Resource, marshal_with
from flask_restful_swagger import swagger

from app.mod_shared.models.db import db
from app.mod_profiles.models import GroupMembershipType
from app.mod_profiles.common.fields.groupMembershipTypeFields import GroupMembershipTypeFields
from app.mod_profiles.common.parsers.groupMembershipType import parser_post
from app.mod_profiles.common.swagger.responses.generic_responses import code_200_ok, code_201_created


class GroupMembershipTypeList(Resource):
    @swagger.operation(
        notes=(u'Retorna todas las instancias existentes de tipo de membresía '
               'de grupo.').encode('utf-8'),
        responseClass='GroupMembershipTypeFields',
        nickname='groupMembershipTypeList_get',
        responseMessages=[
            code_200_ok
        ]
    )
    @marshal_with(GroupMembershipTypeFields.resource_fields, envelope='resource')
    def get(self):
        group_membership_types = GroupMembershipType.query.all()
        return group_membership_types

    @swagger.operation(
        notes=(u'Crea una nueva instancia de tipo de membresía de grupo, y la '
               'retorna.').encode('utf-8'),
        responseClass='GroupMembershipTypeFields',
        nickname='groupMembershipTypeList_post',
        parameters=[
            {
                "name": "name",
                "description": u'Nombre del tipo de membresía de grupo.'.encode('utf-8'),
                "required": True,
                "dataType": "string",
                "paramType": "body"
            },
            {
                "name": "description",
                "description": u'Descripción del tipo de membresía de grupo.'.encode('utf-8'),
                "required": False,
                "dataType": "string",
                "paramType": "body"
            },
        ],
        responseMessages=[
            code_201_created
        ]
    )
    @marshal_with(GroupMembershipTypeFields.resource_fields, envelope='resource')
    def post(self):
        # Obtiene los valores de los argumentos recibidos en la petición.
        args = parser_post.parse_args()
        name = args['name']
        description = args['description']

        new_group_membership_type = GroupMembershipType(name,
                                                        description,
                                                        )
        db.session.add(new_group_membership_type)
        db.session.commit()
        return new_group_membership_type, 201
